...
def episode_urls(podcast_slug: str, pages: int, delay: float) -> list[str]:
    podcast_url = f"{BASE_URL}/podcasts/{podcast_slug}/"
    episode_pattern = re.compile(
        rf"https://www\.ancientfaith\.com/podcasts/{re.escape(podcast_slug)}/([^/?#]+)/"
    )
    seen: set[str] = set()
    urls: list[str] = []
    for page in range(1, pages + 1):
        ...
        for href in parser.links:
            ...
            if episode_pattern.fullmatch(absolute) and absolute not in seen:
                seen.add(absolute)
...
def main() -> None:
    argp = argparse.ArgumentParser(
        description="Fetch transcripts for any Ancient Faith podcast."
    )
    argp.add_argument("podcast_slug", help="Podcast slug as it appears in the Ancient Faith URL, e.g. lordofspirits")
    ...
    urls = episode_urls(args.podcast_slug, args.pages, args.delay)
    written = 0
    skipped = 0
    no_transcript = 0
